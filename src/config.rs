...
impl Config {
    ...
    pub fn new(config_cli: Option<&str>) -> Result<Config> {
        ...
        let mut config_file = File::open(config_file).chain_err(|| "Unable to open config file")?;
        let mut contents = String::new();
        config_file.read_to_string(&mut contents)
            .chain_err(|| "Unable to read config file")?;

        // TODO: Allow overriding by CLI args
        let config = toml::from_str(contents.as_str()).chain_err(|| "Could not parse config file")?;
